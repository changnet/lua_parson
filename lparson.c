...
static JSON_Value *encode_invalid_key_array( lua_State *L,int index,struct lpo *option )
{
    JSON_Status st = 0;
...
static JSON_Value *encode_array( lua_State *L,int index,int max_index,struct lpo *option )
{
    int cur_key = 0;
    JSON_Status st = 0;
    JSON_Value *val = json_value_init_array();
...
static inline JSON_Value *encode_object( lua_State *L,int index,struct lpo *option )
{
...
static JSON_Value *encode_lua_value( lua_State *L,int index,struct lpo *option )
{
    ...
    return max_index > 0 ?
        encode_array( L,index,max_index,option ) :
...
static int encode( lua_State *L )
{
    ...
    str = pretty ?
        json_serialize_to_string_pretty( val ) :
...
static int decode_parson_value(
    lua_State *L,const JSON_Value* js_val,struct lpo *option )
{
    int push_num  = 1;
    int stack_top = lua_gettop( L );
    ...
    switch ( json_value_get_type( js_val ) )
    {
        ...
        case JSONObject:
        {
            ...
            if (option->sparse > 0)
            {
                JSON_Value *sparse_val =
                    json_object_get_value( object,SPARSE_KEY );
                if (sparse_val
                    && JSONBoolean == json_value_get_type( sparse_val )
...
static int decode( lua_State *L )
{
    ...
    val = comment ?
        json_parse_string_with_comments( str ) :
...
static int decode_from_file( lua_State *L )
{
    ...
    val = comment ?
        json_parse_file_with_comments( path ) :
